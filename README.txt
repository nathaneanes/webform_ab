Creating a Test
===============
The Webform A/B Test module creates a new node type (appropriately called "Webform A/B Test"). To build a test, create a Webform A/B Test node as you would with any other content type.
...
### Winning Conditions
These options define the conditions under which a form will be declared the winning form in the Test.

* Minimum Hits: The minimum number of hits that a single form must receive before it will be declared the winner. This is to ensure that a winner is not declared before enough data has been collected to definitively say that one form is better than another. Defaults to 100.
* Win Criteria: There are two options for declaring a winner, and both require a value. Note that in order to be declared the winner of the Test, a form must meet the selected winning condition AND have at least the minimum number of hits specified above.
...
Included webforms are listed on this page, with a link to remove them from the test, and one to edit the form.
...
### Teaser
If only the node teaser should be displayed (for exampe, if the node is promoted to the front page or included in a View), the Public Teaser field is displayed instead of redirecting to one of the included forms.
...
* Name: Name of the webform node.
* Hits: Number of hits to the webform through this Test. By default, the module only logs one hit per user per form, but this option can be changed in the Test Settings (see next section).
...
* Interface updates - for example, enable/disable the hit number and conversion percentage text boxes based on whether Total Conversion or Conversion Percentage is chosen for the Win Criteria.